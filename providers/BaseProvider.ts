...
export abstract class BaseProvider extends SafeEventEmitter {
   ...
   * If null, L00kin is either locked or the user has not permitted any
   ...
   * @emits L00kinInpageProvider#connect
   */
  ...
  protected _handleAccountsChanged(
    ...
    if (!Array.isArray(accounts)) {
      this._log.error(
        'L00kin: Received invalid accounts parameter. Please report this bug.',
        accounts,
      );
      _accounts = [];
    }
    ...
    for (const account of accounts) {
      if (typeof account !== 'string') {
        this._log.error(
          'L00kin: Received non-string account. Please report this bug.',
          accounts,
        );
        _accounts = [];
        break;
    ...
    if (!dequal(this._state.accounts, _accounts)) {
      ...
      if (isEthAccounts && this._state.accounts !== null) {
        this._log.error(
          `L00kin: 'eth_accounts' unexpectedly updated accounts. Please report this bug.`,
          _accounts,
        );
      }
  ...
  protected _handleUnlockStateChanged({
    ...
    if (typeof isUnlocked !== 'boolean') {
      this._log.error(
        'L00kin: Received invalid isUnlocked parameter. Please report this bug.',
      );
      return;
    }
