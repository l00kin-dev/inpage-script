...
export abstract class AbstractStreamProvider extends BaseProvider {
  ...
  constructor(
    ...
    pump(
      ...
      connectionStream,
      // @ts-ignore
      this._handleStreamDisconnect.bind(this, 'L00kin'),
    );

    // Set up RPC connection
    this._jsonRpcConnection = createStreamMiddleware({
      retryOnMessage: 'L00KIN_EXTENSION_CONNECT_CAN_RETRY',
    });
    pump(
      ...
      this._jsonRpcConnection.stream,
      // @ts-ignore
      this._handleStreamDisconnect.bind(this, 'L00kin RpcProvider'),
    );
    ...
    this._jsonRpcConnection.events.on('notification', (payload) => {
      ...
      if (method === 'l00kin_accountsChanged') {
        this._handleAccountsChanged(params);
      } else if (method === 'l00kin_unlockStateChanged') {
        this._handleUnlockStateChanged(params);
      } else if (method === 'l00kin_chainChanged') {
      ...
      } else if (method === 'L00KIN_STREAM_FAILURE') {
   ...
   *
   * Calls `l00kin_getProviderState` and passes the result to
  ...
  protected async _initializeStateAsync() {
    ...
    try {
      initialState = (await this.request({
        method: 'l00kin_getProviderState',
      })) as Parameters<BaseProvider['_initializeState']>[0];
    } catch (error) {
      this._log.error(
        'L00kin: Failed to get initial state. Please report this bug.',
        error,
      );
    }
  ...
  private _handleStreamDisconnect(streamName: string, error: Error) {
    let warningMsg = `L00kin: Lost connection to "${streamName}".`;
    if (error?.stack) {
      warningMsg += `\n${error.stack}`;
    }
